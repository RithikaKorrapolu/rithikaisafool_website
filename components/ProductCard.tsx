...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  // LMSY animation cycle - rotate through images 68-72 (matches home page)
  useEffect(() => {
    if (isLMSYProduct) {
      const interval = setInterval(() => {
        setCcpLayerIndex((prev) => (prev + 1) % 5); // 5 images (68-72)
      }, 1600); // 1.6 seconds per image (matches home page)
        ...
        <div className="relative w-full h-80" style={{ backgroundColor: '#F2F2F2' }}>
          {isLMSYProduct ? (
            <div className="absolute inset-0 flex items-end justify-center">
              {/* Rotating images 68-72 with fade (matches home page) */}
