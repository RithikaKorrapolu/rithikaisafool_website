...
interface BouncingBallPosterProps {
  showLogo?: boolean;
  isHovered?: boolean;
  imageScale?: number; // 0-100, percentage of container size for images
}

export function BouncingBallPoster({ showLogo = false, isHovered = false, imageScale = 100 }: BouncingBallPosterProps = {}) {
  ...
  const imageContainerStyle = imageScale < 100 ? {
    width: `${imageScale}%`,
    height: `${imageScale}%`,
    left: `${(100 - imageScale) / 2}%`,
    top: `${(100 - imageScale) / 2}%`,
  } : {};
