      ...
      <main className="min-h-screen pt-[140px] md:pt-[145px] lg:pt-[155px] pb-20" style={{ backgroundColor: '#F2F2F2' }}>
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          <div className="relative">
            {/* Animated grid lines overlay - desktop (3 columns) */}
            <div className="hidden lg:block absolute inset-0 pointer-events-none z-20">
              <AnimatedGrid columns={3} strokeColor="rgba(0, 0, 0, 0.15)" />
            </div>
            {/* Animated grid lines overlay - tablet (2 columns) */}
            <div className="hidden md:block lg:hidden absolute inset-0 pointer-events-none z-20">
              <AnimatedGrid columns={2} strokeColor="rgba(0, 0, 0, 0.15)" />
            </div>
          ...
          ">
            {reversedPosters.map((poster, posterIndex) => {
              ...
              // Get display name for the poster
              const getPosterDisplayName = (id: number) => {
                const names: { [key: number]: string } = {
                  3: 'TGS Streaming Service',
                  8: 'Client Work',
                  12: 'Quirks',
                  11: 'A Month With You',
                  4: 'Let Me Show You',
                  9: 'Specific Things We Like',
                  13: 'The RIAF Museum of Art',
                  6: 'Voicemail Show',
                  5: 'Condition of the Month',
                  10: 'A Stranger Designed My Sweatshirt',
                };
                return names[id] || poster.label;
              };

              const PosterContent = (
                <div
                  className="poster-wrapper relative border-b border-r border-black/10 md:border-black/0"
                  ref={(el) => {
                    if (el) {
                ...
                >
                  ...
                  <div className="absolute top-4 left-4 z-30">
                    <span
                      className="inline-block px-3 py-1.5 text-xs font-medium uppercase tracking-wider font-[family-name:var(--font-inter)] border border-black/20 bg-white/80"
                      style={{ backdropFilter: 'blur(4px)' }}
                    >
                      {getPosterDisplayName(poster.id)}
                    </span>
                  </div>

                  <div className="h-[108vw] md:h-auto md:min-h-0 flex items-center justify-center">
                  <div
                    className="poster-card flex items-center justify-center overflow-hidden w-[80vw] md:w-full"
                    style={{
                      aspectRatio: '4/5',
                      position: 'relative',
