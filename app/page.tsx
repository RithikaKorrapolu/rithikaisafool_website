...
export default function Home() {
  ...
  const [stwlLastName, setSTWLLastName] = useState('');
  ...
  const handleSTWLSubscribe = async (e: React.FormEvent) => {
    ...
    if (!stwlLastName.trim()) {
      setSTWLMessage('Please enter your last name');
      return;
    }
    ...
    try {
      const response = await fetch('/api/stwl-subscribe', {
        ...
        body: JSON.stringify({ name: stwlName.trim(), lastName: stwlLastName.trim(), phone: stwlPhone, subscribeNewsletter: stwlNewsletter, email: stwlEmail.trim() }),
      });
      ...
      <AnimatePresence>
        {showSTWLPopup && (
          <motion.div
            ...
            className="fixed bg-black/50 flex items-center justify-center z-50 popup-backdrop"
            style={{ top: '-200vh', bottom: '-200vh', left: 0, right: 0 }}
            onClick={() => { setShowSTWLPopup(false); setSTWLName(''); setSTWLLastName(''); setSTWLPhone(''); setSTWLNewsletter(false); setSTWLEmail(''); setSTWLMessage(''); }}
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
            ...
            >
              ...
              <form onSubmit={handleSTWLSubscribe} className="text-center">
                ...
                <input
                  ...
                  value={stwlLastName}
                  onChange={(e) => {
                    setSTWLLastName(e.target.value);
                    setSTWLMessage('');
                  }}
                  className="w-full px-4 py-3 rounded-full border border-black mb-3 bg-transparent font-[family-name:var(--font-inter)] text-black focus:outline-none"
                />
