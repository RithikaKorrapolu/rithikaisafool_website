...
static lv_coord_t tab_page_height = 0;
static lv_coord_t bar_color3_stop = 0;
...
lv_obj_t * lv_demo_music_main_create(lv_obj_t * parent)
{
    ...
    tab_page_height = lv_obj_get_height(parent); // 父容器（Tabview子页面）高度
    bar_color3_stop = BAR_COLOR3_STOP;
    bar_color3_stop = LV_MAX(LV_HOR_RES, tab_page_height) / 3;
    ...
    lv_slider_set_value(slider, PLAYER_VOLUME, LV_ANIM_ON);  
    lv_obj_align(slider, LV_ALIGN_RIGHT_MID, -5, 0); 
    lv_obj_add_flag(slider, LV_OBJ_FLAG_HIDDEN);                                     
    lv_obj_add_event_cb(slider, volume_adjustment_event_cb, LV_EVENT_VALUE_CHANGED, NULL);                                    
    ...
    lv_anim_set_values(&a, LV_MAX(LV_HOR_RES, tab_page_height) / 2, 0);
...
void lv_demo_music_resume(void)
{
    ...
    lv_slider_set_range(slider_obj, 0, lvgl_music_get_track_length(track_id));
    ...
    {
        memset(music_buf,0,sizeof(music_buf));
        sprintf(music_buf,"file://sdcard/%s",lvgl_music_get_title(track_id));
        printf("play:%s\r\n",music_buf);
        Audio_Play_Music(music_buf);
    }
    else if(status == ESP_ASP_STATE_PAUSED)
    {
        printf("恢复播放\r\n");
        Audio_Resume_Play();
    }
...
lv_obj_t * create_cont(lv_obj_t * parent)
{
  ...
  lv_obj_set_size(main_cont, LV_HOR_RES, lv_pct(100));                                               
...
static lv_obj_t * create_title_box(lv_obj_t * parent)
{
    ...
    lv_label_set_text(title_label, lvgl_music_get_title(track_id));
    ...
    lv_label_set_text(artist_label, lvgl_music_get_artist(track_id));
    ...
    lv_label_set_text(genre_label, lvgl_music_get_genre(track_id));*/
...
static lv_obj_t * create_ctrl_box(lv_obj_t * parent)
{
    ...
    icon = lv_imagebutton_create(cont);
    lv_imagebutton_set_src(icon, LV_IMAGEBUTTON_STATE_RELEASED, NULL, &volume, NULL);
    lv_imagebutton_set_src(icon, LV_IMAGEBUTTON_STATE_CHECKED_RELEASED, NULL, &volume, NULL);
    ...
    lv_obj_add_event_cb(icon, volume_click_event_cb, LV_EVENT_CLICKED, NULL);
...
static void track_load(uint32_t id)
{
    ...
    bool next = false;
    if((track_id + 1) % file_count == id) next = true;
...
#else
        lv_event_set_ext_draw_size(e, tab_page_height);
#endif
    }
